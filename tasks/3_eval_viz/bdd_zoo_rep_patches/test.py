...
def main() -> None:
    ...
    if not distributed:
        ...
        outputs = single_gpu_test(
         ...
         # --- Save inference results for later reuse ---
        import pickle, os
        save_dir = args.format_dir or args.work_dir or './results'
        os.makedirs(save_dir, exist_ok=True)
        outputs_file = os.path.join(save_dir, 'outputs.pkl')
        with open(outputs_file, 'wb') as f:
            pickle.dump(outputs, f)
        print(f"\n✅ Inference results saved to: {outputs_file}")
    else:
